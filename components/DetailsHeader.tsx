...
const DetailsHeader = ({ artist, song }) => (
  <section className="relative w-full flex flex-col">
    <div className="w-full bg-gradient-to-l from-transparent to-black sm:h-48 h-28">
      ...
      {song && <img src={song?.album?.images[0]?.url} className="w-full h-48 object-cover opacity-50" />}
    </div>

    <div className="absolute inset-0 flex items-center">
      <img
        alt="art"
        src={artist ? artist?.visuals?.avatarImage?.sources[0]?.url : song?.album?.images[0]?.url}
        className="sm:w-48 w-28 sm:h-48 h-28 rounded-full object-cover border-2 shadow-xl shadow-black"
      />

      <div className={`w-full ml-5 ${artist ? "mt-10" : "mt-2"}`}>
